...
| 77 | `#__postinstall_messages` | `#__postinstall_messages` | `REBUILD` | `GENERATED` | Joomla 6 core/extension manifest identity | compatible extensions installed | — | `EXTENSION` | `G8-06` | `V_REBUILD` | Regenerate target-valid messages from Joomla 6 manifests; source rows are accounting evidence. |
| 78 | `#__utf8_conversion` | migration verification evidence | `DERIVED` | `NONE` | migration batch | completed charset/collation audit | — | — | `G8-07` | `V_XFORM` | Derive completion from successful target encoding checks; do not copy the legacy marker. |
